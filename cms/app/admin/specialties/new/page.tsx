...
async function createSpecialty(payload: SpecialtyPayload) {
  const response = await apiClient.post('/api/admin/specialties', payload);
  if (!response.ok) {
    let message = 'No se pudo crear la especialidad';
    try {
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h3 className="text-2xl font-semibold text-white">Nueva especialidad</h3>
        </div>
        <button
          type="button"
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-border bg-white/5 p-6 text-sm text-muted">
        ...
        {mutation.isError && (
          <p className="text-red-400 text-sm">
            {mutation.error instanceof Error ? mutation.error.message : 'No se pudo crear la especialidad'}
          </p>
        )}
        ...
        <div className="flex gap-3">
          ...
          >
            {mutation.isPending ? 'Guardando...' : 'Crear especialidad'}
          </button>
          <button
            type="button"
            onClick={() => router.push('/admin/specialties')}
